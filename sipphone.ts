 ...
 * @LastEditTime: 2019-08-27 17:58:21
 */
import SIP, { Transport } from 'sip.js'
import { IWS, ISipAccount, ISessionParams, ICallbackObj, IErrorMsg, OutgoingSession, IncomingSession } from './sipphone.declar';
  ...
  private bindIncomingCallSessionEvent(session: IncomingSession) {
    ...
    session.on('trackAdded', () => {
      if (this.currentSessionParams.video) {
        ...
        (remoteVideo as HTMLMediaElement).play();
        ...
        navigator.mediaDevices
          ...
          .then((stream) => {
            ...
            (localVideo as HTMLMediaElement).onloadedmetadata = (e) => {
              (localVideo as HTMLMediaElement).play();
            };
          })
          .catch((err) => {
        ...
        (remoteAudio as HTMLMediaElement).srcObject = remoteStream;
        (remoteAudio as HTMLMediaElement).play();
